...
class GithubAuthModule extends AbstractModule {
  ...
  async init() {
    ...
    auth.unsecureRoute(`${auth.router.path}/github`, 'get');
    auth.unsecureRoute(`${auth.router.path}/github/callback`, 'get');
  ...
  async authenticate(req, res, next) {
    res.redirect('/api/connect/github');
  }
  async callback(req, res, next) {
    if(req.query.error) {
      return next(AuthError.Authenticate(req.query.error));
    }
    try {
